...
public class RenderHeatMap : MonoBehaviour
{
    ...
    [SerializeField] double Q=4000000;
    [SerializeField] double dt=0.1;
    [SerializeField] double theta_x = 4.0 * Math.PI / 15.0;
    [SerializeField] double theta_y = Math.PI/3.0;
    ...
    void Update(){
        ...
        if(!ModeChange()){
            if(IsTrigger.interacrtui){
                for(int i=0;i<25;i++){
                    for(int j=0;j<25;j++){
                        　　
                        double x_0 = MoveGun.RightHandPosition.x * 1000 + 35.0;       //x_0,y_0,z_0=ガンの位置[mm](本来は変数)
                        double y_0 = MoveGun.RightHandPosition.y * 1000 - 125.0;
                        double z_0 = MoveGun.RightHandPosition.z * 1000 + 50.0;
                        double phi_x = MoveGun.RightHandRotation.y;       //ガンの角度（姿勢）
                        double phi_y = MoveGun.RightHandRotation.x + 20.0;
                        ...
                        double sigma_x=((z_0-z__0)/3.0)*(Math.Tan(theta_x/2.0));
                        double sigma_y=((z_0-z__0)/3.0)*(Math.Tan(theta_y/2.0));
